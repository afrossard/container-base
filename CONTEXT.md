...
**Host prerequisite**:
A tool the operator installs on their own machine so this repo's host-side scripts run: the tap-installed launcher (`brew install afrossard/tap/container-base`), `msb`, and `git`, and nothing else.
