...
- **Agent runtime** (`1.4.2-agent`) - the dev image plus its own docker daemon and an entrypoint that starts it, with no dev layer added. What a disposable per-repo agent VM boots as its guest (ADR-0013, ADR-0014).
...
`scripts/launch-agent-runtime` launches the agent runtime image as a microsandbox guest - host-side tooling, and a deliberate, narrow exception to this repo's image-only scope (ADR-0014).
