...
Node, needed only for this repo's own `npm` tooling, resolves via a committed `.mise.toml` (`mise trust && mise install` in `postCreateCommand`) rather than a devcontainer feature, consistent with ADR-0006's "mise owns every language" rule - the dev image already carries `mise` system-wide.
