...
**Status note, added when this document landed on `main`.**
The research below was completed on 2026-08-08 and lived on branch `research/0040-agent-self-merge-controls` while the decision stayed open.
Issue #40 was closed on 2026-09-05 as defense-in-depth deferred: the gap is real but low-severity under the attended-session model of ADR-0017, no control was adopted, and the [closing comment](https://github.com/afrossard/container-base/issues/40#issuecomment-5553343309) names the revisit triggers - sessions going unattended, an actual self-merge, or the credential model changing such that a GitHub App becomes the natural fit anyway.
This document is the full written evaluation that closing comment refers to as available on request, with the follow-up analysis from the session handoff (the machine-account evaluation and the organization-transfer correction) folded into section 5A as that handoff prescribed.
...
Follow-up analysis sharpened two machine-account costs beyond the rotation objection above.

**The token type regresses.**
A fine-grained PAT "is limited to access resources owned by a single user or organization", and GitHub's known-gaps list names the exact case: "Using fine-grained personal access token to contribute to repositories where the user is an outside or repository collaborator."
This repository is owned by `afrossard` and the machine account would be a collaborator, so it would need a **classic PAT with `repo` scope**, regressing the fine-grained decision made in issue #34.
This is the same documented gap the fork analysis in pattern C cites; it is not fork-specific, it applies to any repository the token's owner does not own.
Note also that repositories owned by a personal account have exactly two permission levels, and GitHub documents that collaborators can "create, merge, and close pull requests" - so the machine account still holds merge, and the gate comes entirely from the ruleset's actor axis, not from withheld permission.

**The 2FA failure mode is silent.**
Two-factor authentication is mandatory, including for "unattended or shared access accounts ... such as bots and service accounts".
On lockout, existing tokens "will continue to function" but the account cannot mint new PATs, so the problem surfaces only on rotation day.

**An organization transfer was evaluated and rejected, correcting a recommendation made mid-session.**
An organization changes the repository's _owner_; it grants no second _identity_.
Since this issue's fix is entirely a second identity, an org on its own moves nothing.
Its only bearing was making a machine account's token fine-grained rather than classic, by becoming the resource owner, and that benefit disappears entirely on the GitHub App path, because an App works on a personal repository with no org and no second account, its installation token already acting as the distinct identity `<app>[bot]`.
For completeness, the transfer itself is low risk - issues, pull requests, wiki, stars, watchers, webhooks, secrets, and deploy keys all carry over, and old URLs redirect - but GitHub's transfer documentation is **silent on rulesets**, so if an org is ever revisited for unrelated reasons, verify the ruleset on both sides of the move rather than assuming it survives.
The other reason to decline it: granular roles, org-level PAT approval policies, and token lifetime policies are team governance features, and with one human they are unused surface.
...
### The machine account, held in the pocket

If the approval gate is ever wanted before the credential broker exists, the machine account is the interim answer.
It buys the same identity split as the App, at the cost of a classic `repo`-scope PAT and a second 2FA secret, both accepted as temporary; section 5A carries the full analysis.
It is second-best to the App on every axis except availability, and adopting it should come with an explicit plan to retire it once the broker lands.
