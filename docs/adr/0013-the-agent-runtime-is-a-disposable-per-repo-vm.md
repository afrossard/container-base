...
- **The tool is microsandbox.** Issue #22's experiment ran all three candidates against a draft of the `-agent` image: a full docker daemon runs inside the guest for all three, so the choice turns on what remained after that tie - microsandbox is the only candidate with a directly-verified, working egress allowlist and placeholder-secret mechanism, and the only one documenting all three host platforms this repo might ever need. It is launched with a disk-backed named volume mounted at `/var/lib/docker` (its default nests overlayfs on overlayfs, which fails a real multi-layer build). `apple/container` is the documented fallback (launched with `--cap-add CAP_NET_ADMIN --cap-add CAP_SYS_ADMIN`, since its default capability set is Docker's own restricted one even inside a full VM), and Lima remains the last resort. See `docs/research/0021-vm-microvm-isolation-for-agent-runtimes.md` and `docs/research/0022-vm-tool-experiment-results.md`.
