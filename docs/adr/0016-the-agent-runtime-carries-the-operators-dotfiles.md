...
- **Whether a session can deliver at all now depends on `dotfiles` content.** ADR-0015 makes a pushed branch the only way work leaves a session, and the credential git needs for that push is configuration rather than a token store, so `dotfiles-bootstrap`'s `chezmoi update --apply --force` owns it. `gh auth setup-git` writes that configuration into `~/.config/git/config`, a file `dotfiles` manages, and the next attach deletes it again: `gh` keeps working while `git push` fails with `could not read Username` (issue #182, found verifying #171, the same symptom as the retired #127). The fix ships in `dotfiles` (afrossard/dotfiles#18) as a tracked credential helper, and this repo deliberately adds nothing, because `agent-bringup` runs inside a published image that must stay agnostic of any one forge or agent CLI (ADR-0001). So the trust widening above covers deliverability, not only conventions.
