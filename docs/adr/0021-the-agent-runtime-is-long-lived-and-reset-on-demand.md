...
- **A runtime only picks up a newer published image at reset.** Image freshness and runtime longevity now trade off explicitly, and the operator owns the hygiene of long-lived runtimes; `cleanup-agent-sessions` loses its "all stopped sandboxes are garbage" default, because a stopped pet is the normal state.
- **`workspace-init` must become idempotent.** Resuming a runtime boots it again, so an unconditional `git clone` fails on the second boot.
...
- **Verified so far: filesystem survival across `msb stop`/`msb start`.** Survival across a host reboot is expected (the state is on disk) but not yet observed, and is the one check outstanding before the launcher work lands.
