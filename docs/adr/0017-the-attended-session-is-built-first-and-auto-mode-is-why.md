...
- **The launcher's departures from ADR-0013's disposable-VM model are deliberate and must not be "corrected" toward disposability.** They are the attended shape working as intended; a future reader comparing the launcher against ADR-0013 alone would reasonably conclude it had drifted, and it has not. This originally covered persisted credentials, sandbox reuse across launches, and interactive replace/abort prompts. ADR-0021 has since made reuse the default and folded the replace prompt away, and ADR-0022 removed the credential machinery, so the standing example is the long-lived runtime itself.
- **The accretion risk is real and accepted.** Each affordance added for the attended shape makes the disposable one marginally harder to reach later. The mitigation is keeping the headless path on the stricter defaults - `--persist-claude-auth` once defaulted off whenever a command was given (ADR-0022 has since retired the flag), and the same discipline applies to whatever affordance comes next.
